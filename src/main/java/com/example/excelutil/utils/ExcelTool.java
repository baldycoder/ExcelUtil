package com.example.excelutil.utils;
...
/**
 * @author ：a123145
 * @date ：Created in 2022/7/20 22:11
...
public class ExcelTool {
    ...
    public static List<ErrRows> readExcel(MultipartFile excel, Class rowModel, BaseExcelListener listener) {
        ...
        try {
            ...
            Integer totalSheetCount = reader.excelExecutor().sheetList().size();
     ...
     * 读取 Excel(支持单个model的多个sheet)
     *
     * @param filePath    文件路径 包含文件名+后缀
     * @param rowModel 实体类映射
     * @param listener 用于读取excel的listener
     * @return 错误的行号
     */
    public static List<ErrRows> readExcel(String filePath, Class rowModel, BaseExcelListener listener) {
        ExcelReader reader = getReader(filePath, rowModel, listener);
        try {
            Assert.notNull(reader, "导入Excel失败!");
            Integer totalSheetCount = reader.excelExecutor().sheetList().size();
            for (Integer i = 0; i < totalSheetCount; i++) {
    ...
    private static ExcelReader getReader(MultipartFile excel, Class rowModel,
        ...
        if (!StringUtils.hasText(filename) || (!filename.toLowerCase().endsWith(".xls") && !filename.toLowerCase().endsWith(".xlsx"))) {
     ...
     * 获取 ExcelReader
     *
     * @param filepath    需要解析的 Excel 文件路径，包含文件名
     * @param listener new ExcelListener()
     * @return ExcelReader 或 null
     */
    private static ExcelReader getReader(String filepath, Class rowModel,
                                         BaseExcelListener listener) {

        if (!StringUtils.hasText(filepath) || (!filepath.toLowerCase().endsWith(".xls") && !filepath.toLowerCase().endsWith(".xlsx"))) {
            throw new IllegalArgumentException("文件格式错误！");
        }

        try {
            return EasyExcel.read(filepath, rowModel, listener).build();
        } catch (Exception e) {
            log.error("/*------- 读取Excel IO异常 -------*/", e);
        }
        return null;
